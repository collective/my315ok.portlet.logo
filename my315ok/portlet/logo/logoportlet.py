...
class Renderer(base.Renderer):
    ...
    @memoize
    def image_tag(self):
        portal=self.portalstate.portal()
        ptitle = self.portalstate.portal_title()
        if type(ptitle) == str:
# Ignore errors even if the string is not proper UTF-8 or has
# broken marker bytes.
# Python built-in function unicode() can do this.
            ptitle = unicode(ptitle, "utf-8", errors="ignore")
        else:
# Assume the value object has proper __unicode__() method
        ...
        if imgsrc:
            ...
            return "<img  class='%s' src='%s/%s/@@image' width='%s' height='%s' alt='%s' />" \
                   % ("hack-png",assignment_url,self.data.__name__,str(width),str(height),ptitle)
        return None

    @property
    @memoize
    def site_url(self):
        portal_url=self.portalstate.navigation_root_url()        
        return portal_url       
